...
module.exports = class GoogleAnalyticsEventRider {
  constructor(alexaEvent, config) {
    ...
    if(alexaEvent.request.locale)
      this.visitor.set('ul',alexaEvent.request.locale.toLowerCase());
  ...
  timeEnd(cat,variable) {
    let timer = this.timeRemove(cat,variable);
    if(!timer) return null;
    let elapsed = (+new Date()) - timer.start;
    this.visitor.timing(cat,variable,elapsed);
  }

  timeRemove(cat,variable) {
    let i = _.findIndex(this.timers,{cat,variable});
    if(i<0) return null;
  ...
  logPath() {
    ...
    let name = _(this.path).compact().value().join('; ');
